...
EXECUTABLE = {"permission", "prohibition", "duty"}
HUMAN_ONLY = {"recommendation", "legalNotice", "classification"}
...
def validate_request(request: Any, objects: dict[str, Any], profile: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if request.get("action") not in profile.get("actions", {}):
        errors.append(error("ADUC-POL-ACTION-002", "unknown request action"))
    if request.get("purpose") not in profile.get("purposes", {}):
        errors.append(error("ADUC-POL-PURPOSE-002", "unknown request purpose"))
    if request.get("environment") not in profile.get("environments", {}):
...
def validate_policy(policy: Any, objects: Any, evidence: Any, profile: dict[str, Any]):
    errors: list[dict[str, str]] = []
    ...
    digest = policy.get("targetDigest")
    if not HEX.fullmatch(str(digest or "")):
        errors.append(error("ADUC-POL-TARGET-002", "target digest required"))
    elif target in objects and objects[target].get("digest") != digest:
        errors.append(error("ADUC-POL-TARGET-002", "target digest mismatch"))
    if policy.get("mode") not in profile.get("policyModes", []):
        errors.append(error("ADUC-POL-DOC-001", "invalid mode"))
    if policy.get("disclosure") not in profile.get("disclosureStates", []):
    ...
    if (
        ...
        or not all(iri(item) and item in evidence for item in policy_evidence)
    ):
    ...
    # Both states are mandatory. Missing fields must never be silently promoted.
    if policy.get("conflict") not in {"clear", "contested"}:
        errors.append(error("ADUC-POL-STATE-001", "explicit conflict state required"))
    if policy.get("life") not in {"active", "deprecated"}:
        errors.append(error("ADUC-POL-STATE-001", "explicit lifecycle state required"))
    ...
    if inherited:
        ...
        if (
            ...
            or not all(iri(item) and item in evidence for item in composition_evidence)
        ):
    ...
    for rule in rules:
        ...
        if effect not in profile.get("effects", []):
            errors.append(error("ADUC-POL-RULE-001", "unsupported effect"))
        if effect in EXECUTABLE and rule.get("machineEvaluable") is not True:
            errors.append(error("ADUC-POL-RULE-002", "executable rule not declared"))
        if effect in HUMAN_ONLY and rule.get("machineEvaluable") is not False:
            errors.append(error("ADUC-POL-LEGAL-001", "human statement made executable"))
        if effect in EXECUTABLE:
            action = rule.get("action")
            if not iri(action):
                errors.append(error("ADUC-POL-ACTION-001", "action must be IRI"))
            elif action not in profile.get("actions", {}):
                errors.append(error("ADUC-POL-ACTION-002", "unknown action"))
            elif effect in {"permission", "prohibition"} and profile["actions"][action].get("category") != "primary":
                errors.append(error("ADUC-POL-ACTION-002", "primary action required"))
            elif effect == "duty" and profile["actions"][action].get("category") != "duty":
        ...
        if effect in {"permission", "prohibition"}:
            ...
            elif not all(item in profile.get("purposes", {}) for item in purposes):
                errors.append(error("ADUC-POL-PURPOSE-002", "unknown purpose"))
        elif purposes and (
            not isinstance(purposes, list)
            or not all(iri(item) and item in profile.get("purposes", {}) for item in purposes)
        ):
        ...
        if rule.get("environment") is not None and rule["environment"] not in profile.get("environments", {}):
        ...
        if effect == "duty":
            ...
            if satisfaction_evidence and (
                ...
                or not all(iri(item) and item in evidence for item in satisfaction_evidence)
            ):
        ...
        if claim in {"consent", "legalCompliance", "ownership"}:
            ...
            if (
                ...
                or not all(
                    ...
                    and evidence[item].get("kind") == required_kind
                    and iri(evidence[item].get("provenance"))
    ...
    by_id = {rule.get("id"): rule for rule in normalized if iri(rule.get("id"))}
    for rule in normalized:
        duty_refs = rule.get("duties", [])
        if rule.get("effect") == "permission" and duty_refs and (
            not isinstance(duty_refs, list)
            or not all(iri(item) and item in by_id and by_id[item].get("effect") == "duty" for item in duty_refs)
        ):
...
def duty_satisfied(rule: dict[str, Any], request: dict[str, Any], evidence: dict[str, Any]) -> bool:
    ...
    return bool(required_kind and any(evidence.get(item, {}).get("kind") == required_kind for item in request_evidence))
...
def evaluate(case: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    evidence = case.get("evidence", {})
    objects = case.get("objects", {})
    policy, errors = validate_policy(case.get("policy"), objects, evidence, profile)
    request = case.get("request", {})
    errors += validate_request(request, objects, profile)
    harness_action = case.get("act", {}).get("t")
    if policy is None:
        return {"valid": False, "errors": errors}
    if harness_action == "export":
        if errors:
            return {"valid": False, "errors": errors}
        return {"valid": True, "errors": [], "result": {"policies": 1, "rules": len(policy["rules"]), "jsonld": export_policy(policy)}}
    if harness_action != "evaluate":
    ...
    if request["targetDigest"] != policy["targetDigest"]:
        return {"valid": False, "errors": [error("ADUC-POL-TARGET-002", "target versions differ")]}
    if policy["conflict"] != "clear" or policy["life"] != "active":
        return {"valid": True, "errors": [], "result": {"outcome": "requiresHumanReview", "reason": "policyNotReliablyActive"}}
    if policy["disclosure"] != "complete":
        return {"valid": True, "errors": [], "result": {"outcome": "requiresHumanReview", "reason": "incompleteDisclosure"}}
    if AUTH[policy["auth"]] < AUTH["reviewed"]:
        return {"valid": True, "errors": [], "result": {"outcome": "requiresHumanReview", "reason": "insufficientAuthority"}}
    if not parse_time(policy["valid"]["start"]) <= parse_time(request["at"]) < parse_time(policy["valid"]["end"]):
        return {"valid": True, "errors": [], "result": {"outcome": "deny", "reason": "policyOutsideValidity"}}
    ...
    if prohibitions:
        return {"valid": True, "errors": [], "result": {"outcome": "deny", "reason": "prohibition", "ruleIds": sorted(rule["id"] for rule in prohibitions)}}

    by_id = {rule["id"]: rule for rule in policy["rules"] if iri(rule.get("id"))}
    blocked_pre: set[str] = set()
    for permission in sorted((rule for rule in applicable if rule["effect"] == "permission"), key=lambda item: item["id"]):
        duties = [by_id[item] for item in permission.get("duties", [])]
        unsatisfied_pre = sorted(
            duty["id"]
            for duty in duties
            if duty.get("phase") == "preUse" and not duty_satisfied(duty, request, evidence)
        )
        if unsatisfied_pre:
            blocked_pre.update(unsatisfied_pre)
            continue
        outstanding_post = sorted(
            duty["id"]
            for duty in duties
            if duty.get("phase") == "postUse" and not duty_satisfied(duty, request, evidence)
        )
    ...
    if blocked_pre:
        return {"valid": True, "errors": [], "result": {"outcome": "deny", "reason": "unsatisfiedPreUseDuty", "dutyIds": sorted(blocked_pre)}}
    if any(rule.get("effect") == "legalNotice" for rule in policy["rules"]):
        return {"valid": True, "errors": [], "result": {"outcome": "requiresHumanReview", "reason": "humanOnlyStatement"}}
    if policy["mode"] == "closed":
        return {"valid": True, "errors": [], "result": {"outcome": "deny", "reason": "closedPolicyDefault"}}
...
def run(reference_path: Path, invalid_path: Path) -> dict[str, Any]:
    ...
    profile, errors = registry(reference.get("registry"), reference_path.parent)
    if not profile:
        return {"ok": False, "referenceAccepted": 0, "invalidRejected": 0, "failures": errors}
    cases = {case["id"]: materialize(reference, case) for case in reference["cases"]}
    failures: list[dict[str, Any]] = []
    accepted = rejected = 0
    for case in reference["cases"]:
        actual = evaluate(cases[case["id"]], profile)
        if contains(actual, case["expect"]):
            accepted += 1
        else:
            failures.append({"id": case["id"], "actual": actual, "expected": case["expect"]})
    for case in invalid["cases"]:
        actual = evaluate(patch(cases[case["base"]], case["patch"]), profile)
...
def main() -> int:
    ...
    if arguments.format == "json":
        ...
        print(f"Accepted {result['referenceAccepted']} reference cases.\nRejected {result['invalidRejected']} invalid cases.")
